...
from models.code2seq import reader
from models.code2seq.common import Common
...
class Model:
    ...
    def build_test_graph(self, input_tensors):
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):#self.get_should_reuse_variables()):
            subtoken_vocab = tf.get_variable('SUBTOKENS_VOCAB',
                                             shape=(self.subtoken_vocab_size, self.config.EMBEDDINGS_SIZE),
    ...
    def load_model(self, sess):
        ...
        with open(self.config.LOAD_PATH + '.dict', 'rb') as file:
            ...
            saved_config = pickle.load(file)#MyCustomUnpickler(file).load() 
